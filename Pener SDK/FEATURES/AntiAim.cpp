...
void CAntiAim::do_antiaim(SDK::CUserCmd* cmd)
{
	...
	if ((local_player->GetVelocity().Length2D() < 80) && !(cmd->buttons & IN_JUMP))
	{
		...
		else
		{
			...
			case 8: freestand(cmd);
			case 9: SETTINGS::settings.flip_bool ? cmd->viewangles.y -= 36078 + SETTINGS::settings.aa_realadditive2_val : cmd->viewangles.y += 36078 + SETTINGS::settings.aa_realadditive2_val; break;
			}
		}
	}
